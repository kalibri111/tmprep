    ...
    * Если return_history=False
    ...
    for epoch in range(epochs):
        ...
        for vecs, labels in progress_bar(dataloader):
            ...
            train_loss += loss_function.item() * len(labels)
            sample_num_train += len(labels)
        ...
        if (epoch + 1) % step == 0 or (epoch + 1) == epochs:
            ...
            for vecs, labels in progress_bar(dataloader,
                ...
                sample_num_eval += len(labels)

            train_loss = train_loss / sample_num_train
            train_acc = train_acc / sample_num_eval
            ...
            if valid_dataset:
                ...
                for vecs, labels in progress_bar(valid_dataloader,
                    ...
                    y_pred = model(vecs)
                    valid_loss += hinge_loss(y_pred, labels).item() * len(labels)
